module elliptic_equation_solver
    ...
    delta = eigenvalues_interval(lx,ly,hx,hy,c1,c2,d1,d2)
    xi = delta(1)/delta(2)
    max_iter = log(1.0_mp/eps)/sqrt(4.0_mp*xi)
    rho = (delta(2)-delta(1))/(delta(2)+delta(1))
    u = u0
    N = size(x)-1
    M = size(y)-1
    ...
    subroutine triangle_matrix_method(p, q, f, mu, u0, x, y, hx, hy, lx, ly, c1, c2, d1, d2, max_iter, u, file_id)
    interface    
    pure function p(x,y)
    real(8), intent(in) :: x,y
    real(8) :: p
    ...
    max_iter = log(1.0_mp/eta)/log(1.0_mp/rho)
        ...
        do i=1,N-1
            do j=1,M-1
                ...
                dash_omega(i,j) = (k1*p(x(i)-hx/2.0_mp,y(j))*dash_omega(i-1,j) + &
                        ...
                        & Lh_u_prev + f(x(i),y(j)) / (1 + k1*p(x(i)-hx/2.0_mp,y(j)) + &
                        & k2*q(x(i),y(j))-hy/2.0_mp)
            enddo
        enddo
        do i=N-1,1
            do j=M-1,1
                omega_k(i,j) = (k1*omega_k(i+1,j) + k2*omega_k(i,j+1) + dash_omega(i,j)) /&
                                & (1.0_mp + k1 + k2)
            enddo
        enddo
